...
- [x] **PH7-03 — Correct the malformed LinkedIn profile address in the page footers** — **Priority:** Medium
  ...
  - **Result:** run on 2026-09-21 at `a0589fa`. The owner-confirmed address `https://www.linkedin.com/in/kp-code/` replaces the malformed one in the footer of all 11 root HTML pages, one occurrence per page, with the surrounding markup, link text, `aria-label`, `target`, `rel`, icon and formatting unchanged; the malformed address no longer appears in any of those pages. `npm run qa:source` and `npm run qa:html` passed. `npm run build` passed and all 11 generated `dist/` pages carry the corrected address, with the malformed one absent from `dist/`. `npm run qa:links:external`, run against a local `npm run dev` server, exited `1`: 552 references checked, 54 broken, and every one of those 54 is one of the 30 distinct `https://gastronomy-project-02.netlify.app/` addresses tracked separately as `PH7-04`. No `linkedin.com` address was reported broken, which is a change from the `404` the malformed address returned in `PH7-02`. That clean result is not evidence of a successful response: the checker recorded `https://www.linkedin.com/in/kp-code/` as `status 999`, `state SKIPPED`, and one independent request returned the same `999`. LinkedIn’s bot protection therefore leaves automated reachability unconfirmed, and the correction rests on the owner’s manual browser verification. The build also emitted an unrelated `browserslist` notice that its `caniuse-lite` data is 11 months old.
